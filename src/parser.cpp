             ...
             vector<Face>& triangles)
{
    ifstream file(path);
    if (!file.is_open()) {
        cerr << "[ERROR] Gagal membuka file input: " << path << "\n";
        return false;
    }

    vector<Vector3> rawVertices;
    struct RawFace { int i0, i1, i2; };
    vector<RawFace> rawFaces;
                ...
                const vector<Voxel>& voxels)
{
    ...
    ofstream file(path);
    if (!file.is_open()) {
        cerr << "[ERROR] Gagal membuka file output: " << path << "\n";
        return false;
    }
    ...
    file.close();
    cout << "[INFO] Ekspor selesai: " << voxels.size() << " voxels -> " << path << "\n";
    return true;
}
