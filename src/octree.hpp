...
inline bool triangleIntersectsAABB(const Triangle& tri, const AABB& box) {
...

    {
        ...
        if (triMax < -h.x || triMin > h.x) return false;

        triMin = std::min({v0.y, v1.y, v2.y});
        triMax = std::max({v0.y, v1.y, v2.y});
        if (triMax < -h.y || triMin > h.y) return false;

        triMin = std::min({v0.z, v1.z, v2.z});
        triMax = std::max({v0.z, v1.z, v2.z});
    ...
    VoxelStats& stats)
{
    ...
    if (depth == maxDepth) {
        ...
        stats.totalVoxels.fetch_add(1, std::memory_order_relaxed);
        return node;
    }
    ...
    VoxelStats& stats)
{
    ...
    std::future<ChildResult> futures[8];
    ...
    VoxelStats& stats)
{
    if (vertices.empty() || triangles.empty()) {
        std::cerr << "[WARNING] Tidak ada vertex/triangle untuk diproses.\n";
        return;
    }
    ...
    for (const auto& v : vertices) {
        globalMin.x = std::min(globalMin.x, v.x);
        globalMin.y = std::min(globalMin.y, v.y);
        globalMin.z = std::min(globalMin.z, v.z);
        globalMax.x = std::max(globalMax.x, v.x);
        globalMax.y = std::max(globalMax.y, v.y);
    ...
    double maxSide = std::max({sizeX, sizeY, sizeZ}) * 1.01;
